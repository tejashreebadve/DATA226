...
def extract_data():
    ...
    if response.status_code != 200:
        raise ValueError(f"Failed to fetch data for {SYMBOL}: {response.text}")
    
    data = response.json().get("Time Series (Daily)", {})

    # Extract the last 90 trading days
    last_90_days_data = list(data.items())[:90]  # Assuming API returns in descending order

    return last_90_days_data  # Returning only the stock data
...
def transform_data(raw_data):
    ...
    for date_str, daily_info in raw_data:
        try:
            ...
            print(f"Skipping invalid entry for {SYMBOL} on {date_str}: {str(e)}")
